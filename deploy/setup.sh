...
# 用法：bash setup.sh [--domain your.domain.com] [--port 8000] [--repo URL]
# =============================================================================
set -e
...
info "应用目录：$APP_DIR"
info "服务端口：$APP_PORT"
[[ -n "$DOMAIN" ]] && info "绑定域名：$DOMAIN（将自动申请 HTTPS）" || warning "未指定域名，将使用服务器 IP 访问（仅 HTTP）"
echo ""
...
if [[ -n "$DOMAIN" ]]; then
  ...
  echo ""
  warning "确保域名 $DOMAIN 已解析到本机 IP，否则申请会失败"
  echo ""
  ...
  if [[ -n "$SERVER_IP" && "$DOMAIN_IP" != "$SERVER_IP" ]]; then
    warning "域名 $DOMAIN 当前解析到 $DOMAIN_IP，本机 IP 是 $SERVER_IP"
    warning "DNS 未生效，跳过 HTTPS，稍后可手动执行："
    warning "  sudo certbot --nginx -d $DOMAIN --non-interactive --agree-tos -m admin@$DOMAIN"
  else
    if certbot --nginx -d "$DOMAIN" \
        --non-interactive --agree-tos \
        -m "admin@${DOMAIN}" \
        --redirect 2>&1; then
      success "HTTPS 证书申请成功 ✓（自动续期已启用）"
      # certbot 会自动修改 nginx 配置并 reload
    else
      warning "证书申请失败，保持 HTTP 访问，稍后可手动执行："
...
if grep -q "sk-\.\.\." "$APP_DIR/.env" 2>/dev/null; then
